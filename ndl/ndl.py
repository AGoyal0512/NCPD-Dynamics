...
class Network_Reconstructor():
    def __init__(self,
        ...
        self.dynamics = dynamics ### Dynamics of the network (e.g., opinion dynamics, Kuramoto dynamics)
        if if_tensor_ntwk:
    ...
    def train_dict(self,
        ...
        for t in trange(iter0):
            ...
            if X is not None: # None means no injective homomorphism is sampled
                ...
                if self.dynamics is not None:
                    for color in self.dynamics:
                        seq.append(list(color[emb.astype("int")]))
                        adj.append(X[:,0])
        ...
        graph_factors = [all_factors[j][0].factors[1] for j in range(len(all_factors))]
        ...
        if update_dict_save:
            ...
            self.result_dict.update({'CP Dictionary learned': all_factors})
            self.result_dict.update({'CP Dictionary reconstruction error': reconstruction_list})
            self.result_dict.update({'Adjacency Matrices': np.array(adj)})
            self.result_dict.update({'Dynamics Learned': np.array(seq)})
        ...
        for t in trange(recons_iter):
            ...
            if (ckpt_epoch is not None) and (t % ckpt_epoch == 0) and (t>0):
                ...
                if has_saved_checkpoint:
                    ...
                    for edge in G_recons_new_edges:
                        # edge = eval(edge)
